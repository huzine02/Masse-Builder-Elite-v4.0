...
type AppMode = 'dashboard' | 'workout' | 'victory';
...
const WORKOUT_PLAN: Record<string, ExerciseData[]> = {
  lundi: [
    ...
    { id: 'lun-5', title: 'Dips / Triceps', sets: 3, targetReps: 12, restTime: 90, tempo: "2-0-1-0", videoId: "2z8JmcrW-As" },
  ],
  mercredi: [
...
const MetronomePulse = ({ tempo }: { tempo: string }) => {
  if (!tempo) return null;
  return (
    <div className="w-full h-1 bg-neutral-800 rounded-full mt-1 overflow-hidden">
      <div className="h-full bg-indigo-500/50 animate-pulse-tempo w-full origin-left" style={{ animationDuration: '4s' }} />
    </div>
  );
};

const FocusScreen = ({ timeLeft, onClose }: { timeLeft: number, onClose: () => void }) => {
  const mins = Math.floor(timeLeft / 60);
  const secs = timeLeft % 60;

  return (
    <div className="fixed inset-0 bg-black z-[100] flex flex-col items-center justify-center p-6 text-center animate-enter">
      <h1 className="text-4xl font-black text-red-500 mb-2 tracking-tighter">FOCUS</h1>
      <p className="text-gray-500 text-sm font-bold uppercase tracking-widest mb-12">Lâche ce téléphone</p>

      <div className="text-[120px] font-black font-mono leading-none tabular-nums text-white mb-12">
        {mins}:{secs.toString().padStart(2, '0')}
      </div>

      <div className="space-y-6">
        <p className="text-xl font-bold text-indigo-300">🌬️ Respire par le nez</p>
        <p className="text-xl font-bold text-indigo-300">🚶♂️ Marche un peu</p>
        <p className="text-xl font-bold text-indigo-300">🧠 Visualise la série</p>
      </div>

      <button onClick={onClose} className="absolute bottom-8 text-gray-700 text-xs font-bold uppercase tracking-widest p-4">
...
const Dashboard = ({ day, week, onStart }: { day: Day, week: number, onStart: () => void }) => {
  const today = new Date().toLocaleDateString('fr-FR', { weekday: 'long' });

  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center space-y-8 animate-enter">
      <div className="space-y-2">
        <h2 className="text-gray-500 font-black uppercase tracking-widest text-sm">Bonjour Alex</h2>
        <h1 className="text-5xl font-black italic uppercase text-white leading-none">
          SEMAINE <span className="text-indigo-500">{week}</span>
        </h1>
        <p className="text-xl font-bold capitalize text-gray-400">{today}</p>
      </div>

      <div className="bg-neutral-900/50 p-6 rounded-3xl border border-neutral-800 w-full max-w-sm">
        <h3 className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-2">Objectif du jour</h3>
        <p className="text-white font-bold text-lg">Battre tes performances de la Semaine {Math.max(1, week - 1)}.</p>
        <p className="text-gray-500 text-sm mt-2">"La surcharge progressive est la seule voie."</p>
      </div>

      <button
        onClick={onStart}
        className="w-full max-w-sm bg-white text-black py-6 rounded-2xl font-black text-xl uppercase tracking-widest hover:scale-[1.02] transition-transform shadow-[0_0_30px_rgba(255,255,255,0.2)]"
      >
        COMMENCER LA SÉANCE
      </button>
    </div>
...
const VictoryScreen = ({ volume, onReset }: { volume: number, onReset: () => void }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center space-y-10 animate-enter bg-black">
      <div className="space-y-2">
        <h1 className="text-6xl font-black italic uppercase text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-500 leading-none">
          VICTOIRE
        </h1>
        <p className="text-gray-400 font-bold uppercase tracking-widest">Séance Terminée</p>
      </div>

      <div className="grid grid-cols-1 gap-4 w-full max-w-sm">
        <div className="bg-neutral-900 p-6 rounded-3xl border border-neutral-800">
          <p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Volume Total</p>
          <p className="text-4xl font-black text-white">{volume.toLocaleString()} <span className="text-lg text-gray-600">KG</span></p>
        </div>

        <div className="bg-indigo-900/20 p-6 rounded-3xl border border-indigo-500/30 animate-pulse">
          <p className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-2">Protocole Thermique (44+)</p>
      ...
      <button onClick={onReset} className="text-gray-600 text-xs font-bold uppercase tracking-widest underline">
        Retour au menu
      </button>
    </div>
  );
...
const App = () => {
  ...
  useEffect(() => {
    const initApp = () => {
      const dayMap: Record<number, Day> = { 1: 'lundi', 2: 'mercredi', 3: 'mercredi', 4: 'vendredi', 5: 'vendredi', 6: 'progression', 0: 'progression' };
      const today = new Date().getDay();
      if (dayMap[today]) setActiveTab(dayMap[today]);
      ...
      if (parseInt(localStorage.getItem('currentWeek') || '1') === 1 && calcWeek > 1) setCurrentWeek(calcWeek);
    };
    initApp();
  }, []);
  ...
  const updateVal = (exId: string, idx: number, type: 'w' | 'r' | 'rpe', val: string) => {
    const k = `${exId}-s${idx}`;
    const valStr = workoutData[k] || '||';
    // FIX: Gestion propre des undefined lors du split
    const parts = (typeof valStr === 'string' ? valStr : '||').split('|');
    const w = parts[0] || '';
    const r = parts[1] || '';
    const rpe = parts[2] || '';

    let newVal = '';
    if (type === 'w') newVal = `${val}|${r}|${rpe}`;
    if (type === 'r') newVal = `${w}|${val}|${rpe}`;
    if (type === 'rpe') newVal = `${w}|${r}|${val}`;
  ...
  const getPrevVal = (exId: string, idx: number) => {
    ...
    if (raw && typeof raw === 'string') return raw.split('|');
  ...
  if (mode === 'dashboard') {
    return <Dashboard day={activeTab} week={currentWeek} onStart={() => setMode('workout')} />;
  }

  if (mode === 'victory') {
    return <VictoryScreen volume={getTotalVolume()} onReset={() => setMode('dashboard')} />;
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-white pb-32 max-w-md mx-auto relative overflow-x-hidden selection:bg-indigo-500/30">
      {timeLeft > 0 && <FocusScreen timeLeft={timeLeft} onClose={() => { setTargetTime(null); setTimeLeft(0); }} />}

      {calcWeight && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-6" onClick={() => setCalcWeight(null)}>
          <div className="bg-neutral-900 p-8 rounded-3xl w-full text-center" onClick={e => e.stopPropagation()}>
            <div className="text-6xl font-black mb-4">{calcWeight} <span className="text-xl text-gray-500">kg</span></div>
            <div className="text-indigo-400 font-bold mb-8">({Math.max(0, (calcWeight - 20) / 2)}kg / côté)</div>
            <button onClick={() => setCalcWeight(null)} className="w-full bg-white text-black py-4 rounded-xl font-bold uppercase">Fermer</button>
          </div>
        </div>
      )}
      {activeVideo && (
        <div className="fixed inset-0 bg-black/95 z-[110] flex flex-col justify-center p-4 animate-enter" onClick={() => setActiveVideo(null)}>
          <div className="w-full aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl border border-neutral-800 relative">
            <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${extractVideoId(activeVideo)}?autoplay=1&mute=1&playsinline=1&rel=0&modestbranding=1&controls=1`} title="Demo" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
          </div>
          <button className="mt-8 bg-indigo-600 text-white py-4 rounded-xl font-bold uppercase tracking-widest text-xs" onClick={() => setActiveVideo(null)}>Retour</button>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 bg-black/80 backdrop-blur-xl border-b border-white/5 px-5 py-4 flex justify-between items-center">
        ...
        <button onClick={() => setMode('victory')} className="bg-red-900/20 text-red-500 px-3 py-1 rounded-lg text-xs font-bold uppercase border border-red-500/20">
          FINIR
        </button>
      </header>
      ...
      <main className="flex-1 p-5">
        {activeTab === 'progression' ? (
          ...
          <div className="space-y-8">
            ...
            <div className={`space-y-12 transition-all ${!mcGillDone ? 'opacity-30 blur-sm pointer-events-none' : ''}`}>
              {(WORKOUT_PLAN[activeTab] || []).map((ex) => {
                const isAlt = showAlternatives[ex.id];
                return (
                  <div key={ex.id} className="animate-enter relative">
                    ...
                    {ex.tempo && <MetronomePulse tempo={ex.tempo} />}
                    ...
                    <div className="space-y-3 mt-4">
                      {Array.from({ length: ex.sets }).map((_, i) => {
                        ...
                        return (
                          <div key={i} className="flex gap-2 h-14 items-center">
                            <div className="flex-1 relative group h-full">
                              ...
                              {!w && pW !== '-' && (
                                <button onClick={() => updateVal(ex.id, i, 'w', pW)} className="absolute inset-0 flex items-center justify-center bg-black/50 text-gray-400 opacity-50 z-10"><span className="text-xs font-bold uppercase tracking-widest">↺ {pW}</span></button>
                              )}
                              <button onClick={() => setCalcWeight(parseFloat(w))} className="absolute bottom-2 right-3 text-indigo-500 text-lg">🧮</button>
                            </div>

                            <div className={`flex-[1.4] relative rounded-2xl border flex overflow-hidden h-full ${getRepColor(r, pR)}`}>
                              ...
                              <input
                                ...
                                onBlur={() => { if (r && parseInt(r) > 0 && ex.restTime && timeLeft === 0) startTimer(ex.restTime); }}
                            ...
                            <div className="flex flex-col gap-1 h-full justify-center">
                              <button onClick={() => updateVal(ex.id, i, 'rpe', 'easy')} className={`w-3 h-3 rounded-full border ${rpe === 'easy' ? 'bg-green-500 border-white' : 'bg-green-900/30 border-green-900'}`} />
                              <button onClick={() => updateVal(ex.id, i, 'rpe', 'hard')} className={`w-3 h-3 rounded-full border ${rpe === 'hard' ? 'bg-orange-500 border-white' : 'bg-orange-900/30 border-orange-900'}`} />
                              <button onClick={() => updateVal(ex.id, i, 'rpe', 'fail')} className={`w-3 h-3 rounded-full border ${rpe === 'fail' ? 'bg-red-500 border-white' : 'bg-red-900/30 border-red-900'}`} />
                            </div>
                          </div>
                        );
                      })}
